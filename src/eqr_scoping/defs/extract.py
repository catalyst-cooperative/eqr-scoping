...
def _csvs_to_parquet(
    ...
    try:
        ...
        cid = _extract_ident(
            ...
            parquet_path=_get_output_name(output_path / "ident", ident_path),
    ...
    for file in csv_paths:
        ...
        _extract_other_table(
            csv_path=file,
            parquet_path=_get_output_name(output_path / table_type, ident_path),
            year_quarter=year_quarter,
            cid=cid,
            duckdb_connection=duckdb_connection,
        )
...
def _save_extract_metadata(
    ...
    output_file = (
        base_path / "extraction_metadata" / f"{year_quarter}_extract_metadata.parquet"
    )
    duckdb_connection.execute(
        "COPY ("
